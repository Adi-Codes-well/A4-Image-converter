...
const PhotoEditorControls = ({ alignedImage, backgroundColor, dispatch, loading, error }) => {
  ...
  const processImage = async () => {
    ...
    try {
      ...
      const transparentImageBlob = new Blob([response.data], { type: 'image/png' });
      ...
      img.onload = () => {
        ...
        dispatch({ type: 'SET_PROCESSED_IMAGE', payload: finalCanvas.toDataURL('image/jpeg') });
        dispatch({ type: 'SET_LOADING', payload: false });
        URL.revokeObjectURL(transparentImageURL);
      };
      img.onerror = () => {
        dispatch({ type: 'SET_ERROR', payload: 'Failed to load transparent image.' });
        dispatch({ type: 'SET_LOADING', payload: false });
        URL.revokeObjectURL(transparentImageURL);
      };
      ...
      dispatch({ type: 'SET_ERROR', payload: 'Failed to process image. Please try again.' });
      dispatch({ type: 'SET_LOADING', payload: false });
    }
  };
